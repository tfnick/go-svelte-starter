...
| user clicks create/pay | call `createOrder({ product_id })`, then `createOrderPaymentCheckout(order.id)` |
...
修改 `frontend/src/pages/Users.svelte`、user management API helper、`/users` app route，或后端 `/api/admin/users*` DTO 时遵守本节。
...
Bad: Hard-code `/api/admin/users/${id}/active` directly inside `Users.svelte`; helper tests will not cover it.
...
```js
await fetch(`/api/admin/users/${user.id}/active`, { method: 'PATCH' });
```
...
修改 `frontend/src/pages/Scheduler.svelte`、scheduler/message API helper、或后端 `/api/admin/scheduler/*` / `/api/admin/messages` DTO 时遵守本节。
...
PATCH  /api/admin/scheduler/tasks/:id/enabled
GET    /api/admin/scheduler/tasks/:id/history
GET    /api/admin/messages?queue=<queue>
```
...
* Queue message table is read-only and filters by queue name through `GET /api/admin/messages?queue=...`.
...
Bad: Component manually encodes `/api/admin/scheduler/tasks/${id}` with string concatenation and direct `fetch`; helper tests will not cover it.
...
```js
await fetch(`/api/admin/messages?queue=${queue}`);
```
...
修改 `frontend/src/pages/Events.svelte`、event API helper、`/events` app route，或后端 `/api/admin/events*` DTO 时遵守本节。
...
Bad: Hard-code `/api/admin/events/${id}/deliveries` directly inside `Events.svelte`; helper tests will not cover it.
...
```js
await fetch(`/api/admin/events/${event.id}/deliveries`);
```
...
PUT   /api/admin/dictionary/types/:id
PATCH /api/admin/dictionary/types/:id/enabled
GET   /api/admin/dictionary/types/:type_id/values
POST  /api/admin/dictionary/types/:type_id/values
PUT   /api/admin/dictionary/types/:type_id/values/:id
PATCH /api/admin/dictionary/values/:id/enabled
```
...
Bad: 在 Svelte 组件中直接调用 `/api/admin/parameters/integration-channels`，或在列表中展示具体 `credential_value`；这会绕过 helper 测试和后台配置边界。
...
```svelte
await fetch('/api/admin/parameters/integration-channels?scenario=payment');
```
...
Bad: 在 Svelte 组件中直接调用 `/api/admin/variables`，或把 API key/token 放进 variable value；这会绕过 helper 测试或破坏 secret 边界。
...
* 管理端列表优先使用 `card min-w-0 border border-base-200 bg-base-100 shadow-sm` + `card-body ... p-5`，表格外层使用 `max-w-full overflow-x-auto rounded-box border border-base-200`，表格使用 `table table-zebra table-sm` 并设置稳定 `min-w-*`。选中行用 `bg-primary/5`，空状态用 `rounded-box border border-dashed border-base-200`。
...
* `General` uploads the logo through `uploadSiteLogo(file)` using `FormData`; it must not call `fetch('/api/admin/settings/site/logo')` directly.
* `General` treats `logo_upload_available` from `GET /api/public/settings/site` as authoritative. When it is `false`, the file input and upload button are disabled and the page shows `logo_upload_unavailable_reason`.
...
Good: Admin configures a primary OSS provider, opens `/settings`, selects a PNG/WebP/JPEG logo, uploads it, and the header image switches to `/api/public/settings/logo?v=...`.
...
Bad: Component directly calls `fetch('/api/admin/settings/site/logo')`, or forces `Content-Type: application/json` for `FormData`; this breaks the API helper contract.
...
```svelte
await fetch('/api/admin/settings/site/logo', { method: 'POST', body: formData });
```
...
* 动态 dictionary 通过后端 `/api/public/dictionaries` 获取。
